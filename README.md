...
> interactive mode: each session runs the foreground `spriff wait` loop itself
...
> a callback into the live chat. If you want fully hands-off autonomy instead,
...
- **This live session is the persona.** Run the interactive loop here:
  ...
  not connected back to the same chat, you are not actually in this mode.
